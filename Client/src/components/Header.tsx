...
import { Bell, Menu, Mic, Plus, Search, User, VideoIcon } from "lucide-react";
...
const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
  ...
  const handleSearch = (e: FormEvent) => {
    ...
    if (searchQuerry.trim()) {
      router.push(`/search?q=${encodeURIComponent(searchQuerry.trim())}`);
    }
  };
  ...
  return (
    <header className="flex sticky top-0.5 items-center justify-between p-4 py-2 bg-white text-grey-900 z-2">
      <div className="flex items-center gap-2">
        <Button
          className="buttons block lg:hidden"
          variant="ghost"
          size="icon"
          onClick={onMenuClick}
        >
        ...
        <Link href="/" className="flex items-center gap-1">
          <div className="p-1 rounded">
            {/* <!-- Small screens: only play icon --> */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              id="yt-ringo2-svg_yt10"
              focusable="false"
              aria-hidden="true"
              className="block sm:hidden h-5 w-12"
            >
              <g>
                <path
      ...
      <div className="w-full flex justify-center">
        <form
          ...
          className="flex justify-end items-center flex-1 max-w-3xl"
        >
          <div className="flex flex-1 justify-end">
            <Input
              ...
              placeholder="Search"
              className="rounded-l-full focus-visible:ring-0 hidden sm:block"
            />
            <Button
              type="submit"
              className="buttons rounded-r-full border-white bg-white sm:border sm:bg-gray-50 sm:hover:bg-gray-100 px-2 text-gray-600"
            >
              <Search className="w-5 h-5" />
            </Button>
          </div>
          <Button variant="ghost" size="icon" className="buttons rounded-full">
      ...
      <div className="flex items-center gap-2">
        {user ? (
          <>
            <Button
              className="buttons w-9 sm:w-24 bg-gray-100 rounded-full hover:bg-gray-200"
              variant="ghost"
              size="icon"
            >
              <Plus className="w-10 h-6" />
              <span className="hidden sm:block">
              Create
              </span>
            </Button>
            <Button className="buttons" variant="ghost" size="icon">
              <Bell className="w-6 h-6" />
            </Button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  className="buttons relative h-8 w-8 rounded-full"
                  variant="ghost"
                >
